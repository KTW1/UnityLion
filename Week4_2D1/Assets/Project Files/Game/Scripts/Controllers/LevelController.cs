...
namespace Watermelon.JellyMerge
{
    public class LevelController : MonoBehaviour
    {
        ...
        private IPool cellsBackPool;
        private IPool staticCellsPool;
            ...
            for (int i = 0; i < xLength; i++)
            {
                ...
                for (int j = 0; j < zLength; j++)
                {
                    ...
                    else if (currentColorID == ColorId.Static)
                    {
                        ...
                        staticCellsPool.GetPooledObject().SetPosition(new Vector3(i, 0f, j));
                    ...
                    {
                        activeCubesAmount++;
                        CellBehaviour cell = cellsPool.GetPooledObject().SetPosition(new Vector3(i, 0f, j)).GetComponent<CellBehaviour>();
                    ...
                    cellsBackPool.GetPooledObject().SetPosition(new Vector3(i, 0.01f, j)).SetEulerAngles(new Vector3(90f, 0f, 0));
                }
...
#if UNITY_EDITOR
            Vector2 input = InputController.MoveAction.ReadValue<Vector2>();
            if (input.magnitude > 0)
            {
                OnSwipe(new Vector2Int(Mathf.RoundToInt(input.x), Mathf.RoundToInt(input.y)));
            }
#endif
        }
        ...
        {
            startInput = InputController.MousePosition;
            prevInput = InputController.MousePosition;
        }
        ...
        private void OnSwipe(Vector2Int direction)
        {
            ...
            // Play swipe sound
            AudioController.PlaySound(AudioController.AudioClips.swipeClip);
                ...
                for (int itemOnLine = 0; itemOnLine < itemsInLine; itemOnLine++)
                {
                    ...
                    if (pathLength > 0 && currentItem.ColorID != ColorId.None)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // Play squish sound
                                AudioController.PlaySound(AudioController.AudioClips.squishClip);
        ...
        {
            staticCellsPool.GetPooledObject().SetPosition(new Vector3(indexPosition.x, 0f, indexPosition.y));
